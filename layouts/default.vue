<template>
 <v-app dark>
   ...
   <v-app-bar :clipped-left="clipped" fixed app>
     ...
     <v-spacer />
     <v-toolbar-title v-if="currentAddress"
       >Current account:
       {{
       ...
       <v-col cols="12" align="middle" text-align="center" justify="center" mx-auto>
           ...
           <v-card>
             ...
             <v-text-field
               v-model="TokenUri"
               solo
               label="Ingres aqui el token URI"
               clearable
             ></v-text-field>
             <v-card-actions>
           ...
           <v-card>
             <v-card-title>Obtener NFT</v-card-title>
               ...
               v-model="tokenID"
               solo
               label="Ingres aqui el ID del NFT"
               clearable
             ></v-text-field>
             <v-card-actions>
...
import faces from "@/solidity/artifacts/contracts/Contract.sol/Faces.json";
 ...
 methods: {
   ...
   async getNFts() {
     ...
     let FacesCollectionInstance = new ethers.Contract(
       "0x4Aa10BC3e3C601BfEd24244a7AFcaDC580B6fBCB",
       faces.abi,
       this.signer
     );
